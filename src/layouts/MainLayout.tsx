...
const MainLayout: React.FC<MainLayoutProps> = ({ children, version }) => {
    ...
    return (
        <div className={`adaptive-layout bg-[var(--surface-1)] font-sans text-slate-900 dark:text-slate-100 transition-colors duration-300 ${import.meta.env.VITE_APP_ENV && import.meta.env.VITE_APP_ENV !== 'production' ? 'pt-6 min-h-[calc(100vh-24px)]' : 'min-h-screen'}`}>
            ...
            <aside className={`hidden md:flex flex-col bg-slate-100 dark:bg-slate-900 oled:bg-black border-r border-slate-200 dark:border-slate-800 oled:border-zinc-800 p-4 sticky ${import.meta.env.VITE_APP_ENV && import.meta.env.VITE_APP_ENV !== 'production' ? 'top-6 h-[calc(100vh-24px)]' : 'top-0 h-screen'}`}>
                ...
                <nav className="flex-1 space-y-2 overflow-y-auto custom-scrollbar pr-2">
                    ...
                    <div className="pt-4 mt-4 border-t border-slate-200 dark:border-slate-800 oled:border-zinc-800">
                ...
                <div className="p-4 bg-slate-200/50 dark:bg-slate-800/50 oled:bg-zinc-900/50 rounded-xl mt-auto shrink-0">
            ...
            <main className="flex flex-col relative w-full">
                ...
                <header className="md:hidden sticky top-0 flex items-center justify-between p-4 bg-white/80 dark:bg-slate-900/80 oled:bg-black/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 oled:border-zinc-800 z-30">
                ...
                <div className={`fixed inset-0 z-50 bg-slate-900/20 backdrop-blur-sm md:hidden transition-opacity duration-300 ${mobileMenuOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} onClick={() => setMobileMenuOpen(false)}>
                    <div className={`absolute top-0 right-0 h-full w-72 bg-white dark:bg-slate-900 oled:bg-black p-6 pt-20 shadow-2xl transition-transform duration-300 ease-out ${mobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`} onClick={e => e.stopPropagation()}>
                        {/* User Profile Section */}
                        <div className="mb-6 p-4 bg-slate-50 dark:bg-slate-800/50 oled:bg-zinc-900/50 rounded-xl">
                            <div className="flex items-center gap-3">
