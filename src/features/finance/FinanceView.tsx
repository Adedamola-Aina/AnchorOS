      ...
      <div className={`animate-in fade-in slide-in-from-bottom-8 duration-500 relative ${isMobile ? 'space-y-5' : 'space-y-8'}`}>
        ...
        {accounts.length > 0 && (
          <div className="glass-card overflow-hidden">
            <div className="p-4 border-b border-border-subtle/50 dark:border-border-dark/50 bg-surface-2/30 dark:bg-surface-2-dark/20 flex flex-col sm:flex-row items-center gap-4">
              <div className="flex items-center gap-2 bg-surface-1 dark:bg-surface-1-dark rounded-xl p-1 shadow-sm border border-border-subtle/50 dark:border-border-dark/50">
                ...
                <div className="px-2 flex items-center gap-2 min-w-[160px] justify-center text-sm font-bold text-muted dark:text-muted-dark"><Calendar className="w-4 h-4 text-muted" /><span>{currentMonth.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</span></div>
              ...
              <div className="relative flex-1 w-full"><Search className="w-4 h-4 text-muted absolute left-3 top-1/2 -translate-y-1/2" strokeWidth={2.5} /><input ref={searchInputRef} type="text" placeholder={`Search in ${currentMonth.toLocaleDateString('en-US', { month: 'long' })}...`} className="w-full bg-surface-1 dark:bg-surface-1-dark pl-10 pr-4 py-2.5 rounded-xl border border-border-subtle/50 dark:border-border-dark/50 focus:outline-none focus:ring-2 focus:ring-finance-500/20 text-sm font-medium" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} /></div>
            </div>
