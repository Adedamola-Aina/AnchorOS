...
describe('IDOR: cross-user data access', () => {
  ...
  it('attacker cannot read another user account', async () => {
    const db = authedDb('attacker');
    await assertFails(
      getDoc(doc(db, `${PREFIX}/users/victim/accounts/acct1`))
    );
  });
  ...
  it('attacker cannot delete another user account', async () => {
    const db = authedDb('attacker');
    await assertFails(
      deleteDoc(doc(db, `${PREFIX}/users/victim/accounts/acct1`))
    );
  });
});
...
describe('post-disconnect access revocation', () => {
  beforeEach(async () => {
    await testEnv.withSecurityRulesDisabled(async (ctx) => {
      const db = ctx.firestore();
      // Owner account that was once shared
      await setDoc(doc(db, `${PREFIX}/users/owner/accounts/shared-acct`), {
        ownerId: 'owner',
        name: 'Joint Account',
        sharedWith: {},
      });
  ...
  it('ex-member has no read access after disconnect', async () => {
    const db = authedDb('ex-member');
    await assertFails(
      getDoc(doc(db, `${PREFIX}/users/owner/accounts/shared-acct`))
    );
  });
...
describe('security invariants', () => {
  it('owner field cannot be overwritten by a non-owner', async () => {
    await testEnv.withSecurityRulesDisabled(async (ctx) => {
      const db = ctx.firestore();
      await setDoc(doc(db, `${PREFIX}/users/alice/accounts/acct99`), {
        ownerId: 'alice',
        name: 'Alice Savings',
        sharedWith: {},
      });
    });
    const db = authedDb('mallory');
    await assertFails(
  ...
  it('random collection paths are denied for authenticated users', async () => {
    const db = authedDb('user1');
    await assertFails(
      setDoc(doc(db, 'internal/config'), { secret: 'stolen' })
    );
  });
});
