...
export const seedData = async (userId: string) => {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const currency = crypto.getRandomValues(new Uint32Array(1))[0] / (0xFFFFFFFF + 1) > 0.8 ? 'USD' : 'NGN';
        ...
        const newAccount: AnchorAccount = {
            ...
            balanceCents: Math.floor(crypto.getRandomValues(new Uint32Array(1))[0] / (0xFFFFFFFF + 1) * 800000) + 5000, color: ACCOUNT_COLORS[i],
    ...
    for (let i = 0; i < 60; i++) {
        ...
        const isTransfer = crypto.getRandomValues(new Uint32Array(1))[0] / (0xFFFFFFFF + 1) > 0.85;

        if (isTransfer && accounts.length > 1) {
            const toAccount = accounts.find(a => a.id !== account.id) || accounts[0];
            const amount = Math.floor(crypto.getRandomValues(new Uint32Array(1))[0] / (0xFFFFFFFF + 1) * 50000) + 1000;
            ...
            const type = crypto.getRandomValues(new Uint32Array(1))[0] / (0xFFFFFFFF + 1) > 0.3 ? 'expense' : 'income';
            const title = randomItem(TITLES);
            const category = type === 'income' ? 'Income' : randomItem(CATEGORIES.slice(0, -2));
            const isSharedActivity = account.scope === 'family' && spouseId && crypto.getRandomValues(new Uint32Array(1))[0] / (0xFFFFFFFF + 1) > 0.5;
            const newTxRef = doc(financeRef);
            batch.set(newTxRef, { id: newTxRef.id, title, amountCents: Math.floor(crypto.getRandomValues(new Uint32Array(1))[0] / (0xFFFFFFFF + 1) * 20000) + 500, type, category, accountId: account.id, accountName: account.name, currency: account.currency, scope: account.scope, date: date.toISOString(), createdBy: isSharedActivity ? spouseId : userId, isSoftDeleted: false, accountShares: account.shares || {} });
    ...
    for (let i = 0; i < 11; i++) {
        ...
        const timeOfDay = ['morning', 'afternoon', 'evening', 'any'][Math.floor(crypto.getRandomValues(new Uint32Array(1))[0] / (0xFFFFFFFF + 1) * 4)] as TimeOfDay;
        const taskData: Omit<AnchorTask, 'createdAt'> & { createdAt: FieldValue } = { id: doc(tasksRef).id, title, type: taskType, completed: crypto.getRandomValues(new Uint32Array(1))[0] / (0xFFFFFFFF + 1) > 0.6, category: 'personal', createdAt: serverTimestamp(), domain, reminderTime: '08:00' };
