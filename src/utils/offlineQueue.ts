...
export async function processQueue(processor: (entry: QueueEntry) => Promise<void>): Promise<ProcessResult> {
    const queue = await readQueue();
...
export async function processTaskQueue(processor: (entry: TaskQueueEntry) => Promise<void>): Promise<ProcessResult> {
    const queue = await readTaskQueue();
    if (queue.length === 0) return { succeeded: 0, failed: 0 };
