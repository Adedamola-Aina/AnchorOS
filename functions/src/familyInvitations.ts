...
export const createFamilyInvitation = secureOnCall(
    async (request) => {
        ...
        const { inviteeEmail } = request.data as { inviteeEmail: string };
        const ownerUid = request.auth.uid;
        const ownerEmail = request.auth.token.email;

        if (!ownerEmail) throw new HttpsError('failed-precondition', 'Your email must be verified');
        if (!inviteeEmail) throw new HttpsError('invalid-argument', 'Invitee email is required');
        ...
        if (!existingQuery.empty) throw new HttpsError('already-exists', 'You already have a pending invitation to this email');
        if (await getActiveConnection(ownerUid)) throw new HttpsError('already-exists', 'You already have an active family connection');
...
export const revokeInvitation = secureOnCall(
    async (request) => {
        ...
        if (invite.ownerUid !== ownerUid) throw new HttpsError('permission-denied', 'You can only revoke your own invitations');
        if (invite.status !== 'pending') throw new HttpsError('failed-precondition', 'Only pending invitations can be revoked');
...
export const validateInvitationToken = secureOnCall(
    async (request) => {
        ...
        const inviteDoc = await inviteRef.get();
        if (!inviteDoc.exists) return { valid: false, error: 'Invalid or expired invitation link.' };

        const invite = inviteDoc.data() as FamilyInvitation;
        const now = new Date();
        if (invite.status === 'locked') return { valid: false, error: 'This invitation has been locked due to too many failed attempts.', isLocked: true };
        if (invite.status === 'revoked') return { valid: false, error: 'This invitation has been cancelled by the sender.' };
        if (invite.status === 'accepted') return { valid: false, error: 'This invitation has already been accepted.' };
        if (new Date(invite.expiresAt) < now) return { valid: false, error: 'This invitation has expired.', isExpired: true };
        if (invite.status !== 'pending' && invite.status !== 'awaiting_confirmation') return { valid: false, error: 'This invitation is no longer valid.' };

        return { valid: true, ownerDisplayName: invite.ownerDisplayName, expiresAt: invite.expiresAt, status: invite.status };
    }
);
