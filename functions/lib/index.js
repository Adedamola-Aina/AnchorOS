...
async function createNotification(userId, type, title, message, actorUid, actorName, extra = {}) {
    await db.collection('artifacts').doc(APP_ID)
        ...
        .collection('notifications').add(Object.assign({ type,
        title,
        message,
        actorUid,
        actorName, read: false, dismissed: false, createdAt: admin.firestore.FieldValue.serverTimestamp() }, extra));
}
async function getActiveConnection(userUid) {
    ...
    const ownerQuery = await connectionsRef
        ...
        .get();
    if (!ownerQuery.empty) {
        return Object.assign({ id: ownerQuery.docs[0].id }, ownerQuery.docs[0].data());
    }
    // Check as member
    ...
    if (!memberQuery.empty) {
        return Object.assign({ id: memberQuery.docs[0].id }, memberQuery.docs[0].data());
    }
    return null;
}
const RATE_LIMITS = {
...
// ============================================================================
exports.checkRateLimit = functions.https.onCall(async (data) => {
...
// ============================================================================
exports.createFamilyInvitation = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Authentication required');
    }
    ...
    const recentInvites = ((rateLimitData === null || rateLimitData === void 0 ? void 0 : rateLimitData.attempts) || []).filter((ts) => ts > dayAgo);
    ...
    const ownerDisplayName = ((_a = ownerProfile.data()) === null || _a === void 0 ? void 0 : _a.name) || ownerEmail.split('@')[0];
...
// ============================================================================
exports.validateInvitationToken = functions.https.onCall(async (data) => {
    const { token } = data;
    if (!token) {
...
exports.acceptInvitation = functions.https.onCall(async (data, context) => {
    ...
    // Notify owner
    await createNotification(invite.ownerUid, 'invitation_accepted', 'Family Invitation Accepted', `${context.auth.token.email} has accepted your invitation and entered the correct verification code. Please confirm the connection.`, inviteeUid, context.auth.token.email || 'Unknown');
...
// ============================================================================
exports.confirmConnection = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Authentication required');
    }
    ...
    if (!confirmed) {
        ...
        await createNotification(invite.inviteeUid, 'invitation_rejected', 'Family Connection Declined', `${invite.ownerDisplayName} has declined the family connection.`, ownerUid, invite.ownerDisplayName);
    ...
    const memberDisplayName = ((_a = inviteeProfile.data()) === null || _a === void 0 ? void 0 : _a.name) || invite.inviteeEmail.split('@')[0];
    ...
    // Notify owner
    await createNotification(ownerUid, 'family_connected', '🎉 Family Connected!', `You're now connected with ${memberDisplayName}. No accounts are shared yet. Go to Finance to choose which accounts to share.`, invite.inviteeUid, memberDisplayName);
    // Notify member (persistent notification)
    await createNotification(invite.inviteeUid, 'family_connected', '👥 Family Connected!', `You're now connected to ${invite.ownerDisplayName}'s household. They'll choose which accounts to share with you. Shared accounts will appear in your Finance section once they do.`, ownerUid, invite.ownerDisplayName);
...
// ============================================================================
exports.shareAccount = functions.https.onCall(async (data, context) => {
    var _a;
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Authentication required');
    }
    ...
    const accountName = ((_a = accountDoc.data()) === null || _a === void 0 ? void 0 : _a.name) || 'Unknown Account';
    if (share) {
        ...
        await createNotification(memberUid, 'account_shared', '👥 Account Shared', `${connection.ownerDisplayName} has shared "${accountName}" with you. You can now view and add transactions.`, ownerUid, connection.ownerDisplayName, { accountId, accountName });
        ...
        await createNotification(memberUid, 'account_unshared', 'Account Access Removed', `Access to "${accountName}" has been removed.`, ownerUid, connection.ownerDisplayName, { accountId, accountName });
...
// ============================================================================
exports.getSharedAccountsWithMe = functions.https.onCall(async (_data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Authentication required');
    }
    ...
    const sharedAccounts = snapshot.docs.map(doc => {
        ...
        return {
            ...
            sharedAt: (_b = (_a = data.sharedWith) === null || _a === void 0 ? void 0 : _a[memberUid]) === null || _b === void 0 ? void 0 : _b.grantedAt,
        };
    });
...
// ============================================================================
exports.dismissNotification = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Authentication required');
    }
...
// ============================================================================
exports.disconnectFamily = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Authentication required');
    }
    ...
    await createNotification(otherUid, 'family_disconnected', 'Family Disconnected', isOwner
...
// ============================================================================
exports.onSharedTransactionWrite = functions.firestore
    ...
    if (!(txData === null || txData === void 0 ? void 0 : txData.accountId))
        return null;
    ...
    const sharedWith = (accountData === null || accountData === void 0 ? void 0 : accountData.sharedWith) || {};
    ...
    const actorName = ((_a = actorProfile.data()) === null || _a === void 0 ? void 0 : _a.name) || 'Family member';
    // Format amount
    ...
    for (const sharedUserId of sharedUserIds) {
        ...
        notifications.push(createNotification(sharedUserId, `shared_transaction_${action}`, title, message, userId, actorName, { accountId: txData.accountId, accountName: accountData === null || accountData === void 0 ? void 0 : accountData.name, transactionId }));
        // Audit log
        notifications.push(createAuditLog(`shared_tx_${action}`, userId, {
            transactionId,
 ...
 */
exports.migrateFamilyConnectionsV2 = functions.https.onCall(async (_data, context) => {
 ...
 */
exports.addTransactionToSharedAccount = functions.https.onCall(async (data, context) => {
    var _a, _b;
    if (!context.auth) {
    ...
    const sentOwnerId = data.accountOwnerId;
    if (sentOwnerId) {
    ...
    const hasSharedAccess = (_a = accountData.sharedWith) === null || _a === void 0 ? void 0 : _a[callerUid];
    ...
    const callerName = ((_b = callerDoc.data()) === null || _b === void 0 ? void 0 : _b.name) || 'Family Member';
...
// ============================================================================
exports.fixSharedAccountScopes = functions.https.onCall(async (_data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Authentication required');
    }
    const userId = context.auth.uid;
